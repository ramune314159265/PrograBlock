...
const groups = {
	...
	comparison_expression: ['greater_than', 'greater_than_or_equal', 'less_than', 'less_than_or_equal'],
	arithmetic_expression: ['remainder', 'addition', 'subtraction', 'multiplication', 'division', 'exponentiation'],
}
...
export const assignmentExpressionConverter = {
	...
	toBlocks: {
		binary_expression: (node) => {
			const type = Object.entries(groups).find(([k, v]) => v.includes(node.type))[0]
			return {
				type: type,
				id: node.uid,
				fields: {
